...
def create_class(name, inv_code, max_students, language_id):
    ...
    api_connection.api_post('create_own_cohort', package)


def remove_class(class_id):
    dict = {}
    api_connection.api_post('remove_cohort/' + str(class_id), dict)


def load_class_info(class_id):
    returned_class_infos_string = api_connection.api_get("cohort_info/" + str(class_id)).text
...
def load_classes():
    returned_class_infos_string = api_connection.api_get("cohorts_info").text
...
def load_students(class_id):
    returned_student_infos_string = api_connection.api_get("users_from_cohort/" + str(class_id)).text
...
def verify_invite_code_exists(inv_code):
    inv_code_bool = api_connection.api_get('invite_code_usable/' + str(inv_code)).text
    if inv_code_bool == "OK":
        return False
    return True
