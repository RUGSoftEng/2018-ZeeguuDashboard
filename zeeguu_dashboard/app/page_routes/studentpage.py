...
def student_page(class_id,student_id):
    ...
    if not bookmarks or not info:
        return render_template("empty_student_page.html", info=info, title=info['name'], class_id=class_id,
                               class_name = class_name, student_id=student_id, time=time)
    return render_template("studentpage.html", title=info['name'], info=info, stats=bookmarks, class_id=class_id,
                           class_name=class_name, student_id=student_id, time=time)
